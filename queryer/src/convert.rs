...
// 因为 Rust trait 的孤儿规则，我们如果想对已有类型实现已有的 trait，需要简单包装一下
...
impl<'a> From<Limit<'a>> for usize {
    fn from(l: Limit<'a>) -> Self {
        match l.0 {
            SqlExpr::Value(SqlValue::Number(v, _)) => v.parse().unwrap_or(usize::MAX),
            _ => usize::MAX,
        }
    }
}
...
mod tests {
    ...
    fn parse_sql_works() {
        ...
        let result = Parser::parse_sql(&XlDialect::default(), sql.as_ref());
        assert!(result.is_ok());
        let statement = &result.unwrap()[0];
